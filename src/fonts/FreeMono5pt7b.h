// FreeMono 5pt - generated from GNU FreeFont FreeMono.ttf
// Character dimensions: 6x10 (xAdvance=6, yAdvance=10)
// License: GNU GPL v3 (same as GNU FreeFont)

#pragma once

#include <LovyanGFX.hpp>

const uint8_t FreeMono5pt7bBitmaps[] PROGMEM = {
   };

const lgfx::GFXglyph FreeMono5pt7bGlyphs[] PROGMEM = {
  {     0,   1,   1,   6,    0,    0 },   // 0x20 ' '
  {     1,   2,   6,   6,    2,   -5 },   // 0x21 '!'
  {     3,   4,   3,   6,    1,   -5 },   // 0x22 '"'
  {     5,   4,   7,   6,    1,   -5 },   // 0x23 '#'
  {     9,   4,   8,   6,    1,   -6 },   // 0x24 '$'
  {    13,   4,   6,   6,    1,   -5 },   // 0x25 '%'
  {    16,   4,   5,   6,    1,   -4 },   // 0x26 '&'
  {    19,   2,   3,   6,    2,   -5 },   // 0x27 '''
  {    20,   2,   7,   6,    3,   -5 },   // 0x28 '('
  {    22,   2,   7,   6,    1,   -5 },   // 0x29 ')'
  {    24,   4,   4,   6,    1,   -5 },   // 0x2A '*'
  {    26,   4,   5,   6,    1,   -4 },   // 0x2B '+'
  {    29,   2,   2,   6,    1,    0 },   // 0x2C ','
  {    30,   4,   1,   6,    1,   -2 },   // 0x2D '-'
  {    31,   2,   1,   6,    2,    0 },   // 0x2E '.'
  {    32,   4,   8,   6,    1,   -6 },   // 0x2F '/'
  {    36,   4,   6,   6,    1,   -5 },   // 0x30 '0'
  {    39,   4,   6,   6,    1,   -5 },   // 0x31 '1'
  {    42,   4,   6,   6,    1,   -5 },   // 0x32 '2'
  {    45,   4,   6,   6,    1,   -5 },   // 0x33 '3'
  {    48,   4,   6,   6,    1,   -5 },   // 0x34 '4'
  {    51,   4,   6,   6,    1,   -5 },   // 0x35 '5'
  {    54,   4,   6,   6,    1,   -5 },   // 0x36 '6'
  {    57,   4,   6,   6,    1,   -5 },   // 0x37 '7'
  {    60,   4,   6,   6,    1,   -5 },   // 0x38 '8'
  {    63,   4,   6,   6,    1,   -5 },   // 0x39 '9'
  {    66,   2,   4,   6,    2,   -3 },   // 0x3A ':'
  {    67,   3,   5,   6,    1,   -3 },   // 0x3B ';'
  {    69,   4,   5,   6,    1,   -4 },   // 0x3C '<'
  {    72,   4,   2,   6,    1,   -3 },   // 0x3D '='
  {    73,   4,   5,   6,    1,   -4 },   // 0x3E '>'
  {    76,   4,   6,   6,    1,   -5 },   // 0x3F '?'
  {    79,   4,   7,   6,    1,   -5 },   // 0x40 '@'
  {    83,   6,   6,   6,    0,   -5 },   // 0x41 'A'
  {    88,   5,   6,   6,    0,   -5 },   // 0x42 'B'
  {    92,   4,   6,   6,    1,   -5 },   // 0x43 'C'
  {    95,   5,   6,   6,    0,   -5 },   // 0x44 'D'
  {    99,   5,   6,   6,    0,   -5 },   // 0x45 'E'
  {   103,   5,   6,   6,    0,   -5 },   // 0x46 'F'
  {   107,   5,   6,   6,    1,   -5 },   // 0x47 'G'
  {   111,   5,   6,   6,    1,   -5 },   // 0x48 'H'
  {   115,   4,   6,   6,    1,   -5 },   // 0x49 'I'
  {   118,   5,   6,   6,    1,   -5 },   // 0x4A 'J'
  {   122,   6,   6,   6,    0,   -5 },   // 0x4B 'K'
  {   127,   4,   6,   6,    1,   -5 },   // 0x4C 'L'
  {   130,   6,   6,   6,    0,   -5 },   // 0x4D 'M'
  {   135,   6,   6,   6,    0,   -5 },   // 0x4E 'N'
  {   140,   4,   6,   6,    1,   -5 },   // 0x4F 'O'
  {   143,   5,   6,   6,    0,   -5 },   // 0x50 'P'
  {   147,   4,   7,   6,    1,   -5 },   // 0x51 'Q'
  {   151,   6,   6,   6,    0,   -5 },   // 0x52 'R'
  {   156,   4,   6,   6,    1,   -5 },   // 0x53 'S'
  {   159,   4,   6,   6,    1,   -5 },   // 0x54 'T'
  {   162,   6,   6,   6,    0,   -5 },   // 0x55 'U'
  {   167,   6,   6,   6,    0,   -5 },   // 0x56 'V'
  {   172,   6,   6,   6,    0,   -5 },   // 0x57 'W'
  {   177,   6,   6,   6,    0,   -5 },   // 0x58 'X'
  {   182,   4,   6,   6,    1,   -5 },   // 0x59 'Y'
  {   185,   4,   6,   6,    1,   -5 },   // 0x5A 'Z'
  {   188,   1,   7,   6,    3,   -5 },   // 0x5B '['
  {   189,   4,   8,   6,    1,   -6 },   // 0x5C '\'
  {   193,   1,   7,   6,    2,   -5 },   // 0x5D ']'
  {   194,   4,   2,   6,    1,   -5 },   // 0x5E '^'
  {   195,   6,   1,   6,    0,    1 },   // 0x5F '_'
  {   196,   1,   1,   6,    2,   -5 },   // 0x60 '`'
  {   197,   4,   4,   6,    1,   -3 },   // 0x61 'a'
  {   199,   5,   6,   6,    0,   -5 },   // 0x62 'b'
  {   203,   4,   4,   6,    1,   -3 },   // 0x63 'c'
  {   205,   5,   6,   6,    1,   -5 },   // 0x64 'd'
  {   209,   4,   4,   6,    1,   -3 },   // 0x65 'e'
  {   211,   4,   6,   6,    1,   -5 },   // 0x66 'f'
  {   214,   5,   6,   6,    1,   -3 },   // 0x67 'g'
  {   218,   6,   6,   6,    0,   -5 },   // 0x68 'h'
  {   223,   4,   6,   6,    1,   -5 },   // 0x69 'i'
  {   226,   3,   8,   6,    1,   -5 },   // 0x6A 'j'
  {   229,   4,   6,   6,    1,   -5 },   // 0x6B 'k'
  {   232,   4,   6,   6,    1,   -5 },   // 0x6C 'l'
  {   235,   6,   4,   6,    0,   -3 },   // 0x6D 'm'
  {   238,   4,   4,   6,    1,   -3 },   // 0x6E 'n'
  {   240,   4,   4,   6,    1,   -3 },   // 0x6F 'o'
  {   242,   5,   6,   6,    0,   -3 },   // 0x70 'p'
  {   246,   5,   6,   6,    1,   -3 },   // 0x71 'q'
  {   250,   4,   4,   6,    1,   -3 },   // 0x72 'r'
  {   252,   4,   4,   6,    1,   -3 },   // 0x73 's'
  {   254,   5,   6,   6,    0,   -5 },   // 0x74 't'
  {   258,   5,   4,   6,    0,   -3 },   // 0x75 'u'
  {   261,   6,   4,   6,    0,   -3 },   // 0x76 'v'
  {   264,   6,   4,   6,    0,   -3 },   // 0x77 'w'
  {   267,   4,   4,   6,    1,   -3 },   // 0x78 'x'
  {   269,   4,   6,   6,    1,   -3 },   // 0x79 'y'
  {   272,   4,   4,   6,    1,   -3 },   // 0x7A 'z'
  {   274,   2,   7,   6,    2,   -5 },   // 0x7B '{'
  {   276,   1,   7,   6,    3,   -5 },   // 0x7C '|'
  {   277,   2,   7,   6,    2,   -5 },   // 0x7D '}'
  {   279,   4,   1,   6,    1,   -2 } }; // 0x7E '~'

const lgfx::GFXfont FreeMono5pt7b PROGMEM = {
  (uint8_t  *)FreeMono5pt7bBitmaps,
  (lgfx::GFXglyph *)FreeMono5pt7bGlyphs,
  0x20, 0x7E, 10 };
