...
- **`screen.acquire_wakelock(tag)`** / **`screen.release_wakelock(tag)`**
  -- tag-keyed counter that pins the idle ladder at stage 0 regardless
  of `ss_timeout`. Multiple distinct tags coexist; the ladder resumes
  only once the last tag is released. `release_wakelock` resets
  `last_input_time` so a long-held wakelock doesn't jump the next idle
  tick straight to panel-off. Implicit wakelock built in:
  `_wakelocks_held()` polls `ez.audio.is_recording()`, so voice-notes
  / signal-test capture paths stay lit without explicit acquire.
  Prefer wakelocks over per-frame `notify_input()` for background
  activities that need the display alive for an unbounded duration.
