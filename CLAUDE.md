...
10. **ui_sounds** — UI sound effects via the audio engine
11. **notifications** — Toast queue + bus subscribers for OTA, DMs,
...
12. **apps** — Registered file-type → screen handlers (used by the file manager)
13. **gps** — `gps_svc.start_sync_loop()` is always called; the loop itself
    respects the user's "never / at boot / hourly" pref and is a no-op when
    GPS is disabled
14. **power** — `power_svc.start()` spawns a 30 s battery poll that
...
(`dm`, `file`, `battery`, `sd`, `ota`, `channel`, `rxn`, `system`).
Note that `notify_words` lives in the same `notify_*` keyspace but is
