...
function SystemInfo:render(display)
    ...
    -- Chip model
    local chip = ez.system.chip_model and ez.system.chip_model() or "ESP32-S3"
    display.draw_text(x, y, "Chip: " .. chip, colors.TEXT)
    y = y + fh

    -- CPU frequency
    local cpu_freq = ez.system.cpu_freq and ez.system.cpu_freq() or 240
    display.draw_text(x, y, string.format("CPU: %d MHz", cpu_freq), colors.TEXT)
    y = y + fh

    -- Heap memory
    local heap = math.floor(ez.system.get_free_heap() / 1024)
    local total_heap = ez.system.get_total_heap and math.floor(ez.system.get_total_heap() / 1024) or 0
    ...
    -- PSRAM
    local psram = math.floor(ez.system.get_free_psram() / 1024)
    local total_psram = ez.system.get_total_psram and math.floor(ez.system.get_total_psram() / 1024) or 0
    ...
    -- Uptime
    local uptime = ez.system.uptime and ez.system.uptime() or math.floor(ez.system.millis() / 1000)
    display.draw_text(x, y, string.format("Uptime: %d seconds", uptime), colors.TEXT)
    y = y + fh

    -- Battery
    local battery = ez.system.get_battery_percent()
